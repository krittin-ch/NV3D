/*
Batch version of three-NN, modified from the original implementation of Shaoshuai Shi's codes.
...
__global__ void compute_three_nn_kernel_fast(int b, int n, 
    ...
    float best_d[K] = {FLT_MAX, FLT_MAX, FLT_MAX};
    int64_t best_idx[K] = {-1, -1, -1};
...
void compute_three_nn_kernel_launcher_fast(const float *db_points, 
    int b, int n, 
    ...
    compute_three_nn_kernel_fast<<<blocks, threads>>>(b, n, dq_points, db_points, dist2, idx, db_mask);
